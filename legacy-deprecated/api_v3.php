...
function fileExistsOnCommons ( $filename ) {
	$url = "http://commons.wikimedia.org/w/api.php?action=query&format=json&titles=File:" . urlencode(str_replace(' ','_',ucfirst(trim($filename)))) ;
...
if ( isset($_REQUEST['oauth_verifier']) ) redirect2url ( 'http://wikishootme.toolforge.org/' ) ;
...
if ( $action == 'check' ) {
	...
	$oa->doAuthorizationRedirect('http://wikishootme.toolforge.org/api_v3.php');
	exit(0);
	...
	// Fin
	$url = "http://commons.wikimedia.org/wiki/File:".$tfc->urlEncode($new_file_name) ;
	if ( $action == 'upload' ) {
	...
	$widar->authorization_callback = 'http://wikishootme.toolforge.org/api_v3.php' ;
