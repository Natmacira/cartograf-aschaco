...
var wikishootme = {
	...
	sparql_url : 'http://query.wikidata.org/bigdata/namespace/wdq/sparql' ,
	...
	tile_layers : {
		osm: {name:'OSM (WMF)',url:'http://maps.wikimedia.org/osm-intl/{z}/{x}/{y}.png',attribution:'&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'} ,
		esri_wm: {name:'ESRI WorldMap',url:'http://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',attribution:'Tiles &copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community'} ,
		esri_topo: {name:'ESRI TopoMap',url:'http://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}',attribution:'Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ, TomTom, Intermap, iPC, USGS, FAO, NPS, NRCAN, GeoBase, Kadaster NL, Ordnance Survey, Esri Japan, METI, Esri China (Hong Kong), and the GIS User Community'} ,
		stamen_terrain: {name:'Stamen Terrain',url:'http://stamen-tiles-{s}.a.ssl.fastly.net/terrain/{z}/{x}/{y}.png',subdomains:'abcd',attribution:'Map tiles by <a href="http://stamen.com">Stamen Design</a>, <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a> &mdash; Map data &copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'} ,
	} ,
	colors : {
		me:'#888888',
	...
	updateSearchLinks : function () {
		...
		var url = "http://fist.toolforge.org/wdfist/index.html?sparql=" ;
		...
		url = "http://fist.toolforge.org/fist/check_flickr_geo.php?lat="+center.lat+"&lon="+center.lng+"&radius="+radius_km+"&doit=1" ;
	...
	createImageThumbnail : function ( image ) {
		var me = this ;
		var url = 'http://commons.wikimedia.org/wiki/Special:Redirect/file/'+escattr(encodeURIComponent(image))+'?width='+me.thumb_size+'px&height='+me.thumb_size+'px' ;
		var h = '' ;
		h += "<div class='thumb'>" ;
		h += "<a target='_blank' href='http://commons.wikimedia.org/wiki/File:" + escattr(encodeURIComponent(image)) + "'>" ;
	...
	createPopup : function ( entry ) {
		...
		if ( typeof entry.mixnmatch != 'undefined' ) {
			if ( entry.mixnmatch.q != null ) {
				...
				var qlink = "<a href='http://www.wikidata.org/wiki/Q" + q + "' target='_blank'>Q" + q + "</a>" ;
...
//			h += "</div>" ;
		...
		if ( typeof me.marker_me != 'undefined' ) {
			...
			h += "<a target='_blank' href='http://maps.google.co.uk/maps?dirflg=w&saddr="+pos.lat+","+pos.lng+"&daddr="+entry.pos[0]+","+entry.pos[1]+"'>"+me.tt.t('route')+"</a>" ;
			h += "</div>" ;
		}
...
//		me.tt.updateInterface($('#dropdownUploadsLi')) ;
	...
	uploadNext : function () {
		...
		.then ( function () {
			...
			// Logging
			$.getJSON ( 'http://tools.wmflabs.org/magnustools/logger.php?tool=wikishootme&method=file uploaded&callback=?' , function(j){} ) ;
		} ) ;
	...
	addWikimediaEntry : function ( mode , server , v ) {
		...
		var i = {
			...
			page : v.title ,
			mode : mode ,
			url : 'http://' + server + '/wiki/' + encodeURIComponent(v.title) ,
			server : server ,
			ns : v.ns
		} ;
	...
	transferFlickr2Commons : function ( popup , flickr_id ) {
		...
		wsm_comm.getFlinfo ( params , function ( d ) {
			...
			var comment = 'Transferred from Flickr via [http://wikishootme.toolforge.org WikiShootMe] #wikishootme' ;
	...
	loadFlickrLayer2 : function () {
		...
		var me = this ;
		me.loadCachedJSON ( 'http://api.flickr.com/services/rest/' , params , function ( d ) {
			me.flickr_pics = [] ;
			...
			$.each ( d.photos.photo , function ( k , v ) {
				...
				var i = {
					...
					url:'http://www.flickr.com/photos/'+v.owner+'/'+v.id
				} ;
...
//		console.log ( params ) ;
		...
		me.loadCachedJSON ( 'http://mix-n-match.toolforge.org/api.php?callback=?' , params , function ( d ) {
//			console.log ( d ) ;
			...
			$.each ( d.data , function ( k , v ) {
				var i = {
					...
					url:'http://mix-n-match.toolforge.org/#/entry/'+v.entry_id
				} ;
...
//		console.log ( params ) ;
		...
		me.loadCachedJSON ( 'http://mix-n-match.toolforge.org/lc_api.php?callback=?' , params , function ( d ) {
//			console.log ( d ) ;
			me.mnm_lc_entries = [] ;
		...
		me.loadCachedJSON ( me.sparql_url , {
			...
			$.each ( d.results.bindings , function ( dummy , item ) {
				...
				var i = {
					...
					label:q ,
					mode:'wikidata' ,
					url:'http://www.wikidata.org/wiki/' + q ,
					ns:0
				} ;
	...
	createMap : function () {
		...
		me.map.on ( 'popupopen' , function ( pe ) {
			...
			c.find('div.pageimage_toload').each ( function () { // Lazy-load Commons image
				...
				var url = 'http://'+server+'/w/api.php?callback=?' ;
				$.getJSON ( url , {
					action:'mobileview',
					page:page,
					prop:'image',
	...
	init : function () {
		...
		// Logging
		$.getJSON ( 'http://tools.wmflabs.org/magnustools/logger.php?tool=wikishootme&method=loaded&callback=?' , function(j){} ) ;
